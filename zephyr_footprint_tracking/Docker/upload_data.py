...
def parse_args():
    ...
    parser.add_argument("-u", "--database-url", help="Database DSN", required=True)
...
def process_files(data_dir, zephyr_base, dry_run):
    ...
    for hash in os.listdir(f'{data_dir}'):
        ...
        for path in Path(args.data).rglob('*.json'):
            ...
            if 'ram' in path.suffix:
                typ = 'ram'
            else:
                typ = 'rom'

            (commit, app, feature, board, name) = path.relative_to(args.data).parts
